...
import {Col, ColorPicker, Input, Radio, Row, Select} from 'antd';
...
const StyleShapeM: React.FC<Props> = ({setToggleModal, setStyleShape}: Props): JSX.Element => {
    ...
    const [borderSize, setBorderSize] = useState<any>(null);
    const [background, setBackground] = useState<any>(null);
    const [borderColor, setBorderColor] = useState<any>(null);

    useEffect(() => {
        let id = globalStore.itemAnnotationStyle.id;
        let type = globalStore.itemAnnotationStyle.type;
        if (type === 'rect-polygon') {
            setBorderSize(globalStore.getRectPolygonPalletById(id).weight)
            setBackground(globalStore.getRectPolygonPalletById(id).fillColor)
            setBorderColor(globalStore.getRectPolygonPalletById(id).color)
        } else if (type === 'circle-polygon') {
            setBorderSize(globalStore.getCirclePolygonPalletById(id).weight)
            setBackground(globalStore.getCirclePolygonPalletById(id).fillColor)
            setBorderColor(globalStore.getCirclePolygonPalletById(id).color)
        }
        console.log(borderSize, background, borderColor)
    }, [globalStore.itemAnnotationStyle.id])
    ...
    return (<>
            {borderSize && background && borderColor &&
