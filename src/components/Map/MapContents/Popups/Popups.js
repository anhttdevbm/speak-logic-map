...
export const functionPopup = (map, setModal, setModalType, isLocked, e) => {
  ...
  const popup = L.popup([e.latlng.lat, e.latlng.lng], {content: fnPopupHTML(name, type, error, hasBoundary, isFlashing), offset: L.point(0, e.latlng.lat < 20 ? 0 : 440)});
...
export const personPopup = (map, setModal, setModalType, isLocked, e) => {
  clearAllPopups(map);
  const popup = L.popup([e.latlng.lat, e.latlng.lng], {content: personPopupHTML(), offset: L.point(0, e.latlng.lat < 20 ? -10 : 200)});
  popup.addTo(map);
